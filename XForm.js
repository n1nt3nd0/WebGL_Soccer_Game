...
function XForm() {
	...
	this.findMatrix = function() {
		var posMat = TraslateMatrixWithVector(this.pos);
		var ctrMat = TraslateMatrixWithVector(this.ctr);
		var negCtrMat = TraslateMatrixWithVector(vectorMult(this.ctr, -1));
	...
	this.findInverseMatrix = function() {
		...
		var negPosMat = TraslateMatrixWithVector(vectorMult(this.pos, -1));
		var ctrMat = TraslateMatrixWithVector(this.ctr);
		var negCtrMat = TraslateMatrixWithVector(vectorMult(this.ctr, -1));
		var invScaleMat = ScaleMatrix(1/this.scale[0], 1/this.scale[1], 1/this.scale[2]);
